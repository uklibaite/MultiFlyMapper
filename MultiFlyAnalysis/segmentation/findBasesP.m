function [basis1 basis2] = findBasesP(inputImages,parameters)
% returns body basis images given a set of test images including 2 flies
    ...
    fly1Ims = alignedAll(:,:,areasIdx(1:300));
    fly2Ims = alignedAll(:,:,areasIdx(301:600));
    fly1Med = median(fly1Ims,3);
    fly2Med = median(fly2Ims,3);
    fly1Body = fly1Med>parameters.bodyThreshold;
    fly2Body = fly2Med>parameters.bodyThreshold;
    fly1NonBody = fly1Med>parameters.minImageValue & fly1Med<parameters.bodyThreshold;
    fly2NonBody = fly2Med>parameters.minImageValue & fly2Med<parameters.bodyThreshold;
    basis1 = cell(1,2); basis2 = cell(1,2);
    f1 = find(sum(fly1Body)~=0,1,'last')-15;
    f2 = find(sum(fly2Body)~=0,1,'last')-15;
    cut1 = zeros(size(fly1Body)); cut1(:,1:f1) = ones(size(fly1Body,1),f1);
    cut2 = zeros(size(fly2Body)); cut2(:,1:f2) = ones(size(fly2Body,1),f2);
    cut1B = imclose(fly1Body+fly1NonBody>=1,ones(15,15));
    cut2B = (fly2Body+fly2NonBody+cut2)>=1;
    basis1{1} = double(fly1Body); basis1{2} = double(fly1NonBody); basis1{3} = double(cut1B);
    basis2{1} = double(fly2Body); basis2{2} = double(fly2NonBody); basis2{3} = double(cut2B);
