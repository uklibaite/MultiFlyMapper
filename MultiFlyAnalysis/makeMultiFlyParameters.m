...
%**********Alignment Parameters*************
    ...
    if ~isfield(parameters,'imageLength') || isempty(parameters.imageLength)
        parameters.imageLength = imageLength;
    end
    
    if ~isfield(parameters,'numZeros') || isempty(parameters.numZeros)
        parameters.numZeros = numZeros;
    end
    ...
    if ~isfield(parameters,'backgroundImages') || isempty(parameters.backgroundImages) || length(parameters.backgroundImages) < 2
        ...
        parameters.backgroundImages{1} = imread('./segmentation/female_bg.tiff');
        parameters.backgroundImages{2} = imread('./segmentation/male_bg.tiff');
    end
